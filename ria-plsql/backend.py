...
def submit_request_proc():
    ...
    try:
        cursor.callproc('SubmitPrintRequest', 
                        [user_id, file_name, copies, color, pages])
...
@app.route('/user_requests_history/<int:user_id>', methods=['GET'])
def user_requests_history(user_id):
    conn   = get_db_connection()
    cursor = conn.cursor(dictionary=True)

    try:
        cursor.callproc('GetUserRequestHistory', [user_id])
        for result in cursor.stored_results():
            data = result.fetchall()
        return jsonify(data)
    except Exception as e:
...
@app.route('/update_status_proc', methods=['POST'])
def update_status_proc():
    data   = request.json
    req_id = data['req_id']
    status = data['status']
    ...
    try:
        cursor.callproc('UpdateRequestStatus', [req_id, status])
        for result in cursor.stored_results():
            msg = result.fetchall()
        conn.commit()
        return jsonify({"message": "Status updated successfully"})
...
def admin_dashboard():
    ...
    try:
        cursor.callproc('GetAdminDashboard')
        results = []
        for result in cursor.stored_results():
            results.append(result.fetchall())
        return jsonify({
            "status_summary" : results[0],
            "cost_per_user"  : results[1]
        })
    except Exception as e:
        return jsonify({"error": str(e)}), 500
    finally:
...
def calculate_cost():
    ...
    try:
        cursor.execute(
            "SELECT CalculatePrintCost(%s, %s, %s) AS cost",
            (copies, color, pages)
        )
        result = cursor.fetchone()
        return jsonify({"estimated_cost": float(result[0])})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
    finally:
        cursor.close()
